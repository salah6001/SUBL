# MindType AI — Integration Spec: `data_and_model/` ↔ `.NET Backend`

## Architecture

```
Desktop Agent                 ONEX API (.NET)               Python ML Service
(subl_agent)                   (backend/)                    (data_and_model/mindtype-ai/)
     │                              │                               │
...
| Aspect | Detail |
|---|---|
| File | `data_and_model/mindtype-ai/model/mindtype_model.pkl` |
...
services:
  ...
  ml-service:
    ...
    build:
      context: ../data_and_model/mindtype-ai
...
| File | Role |
|---|---|
| `data_and_model/mindtype-ai/backend/main.py` | FastAPI app: `/predict-stress` + `/health` |
| `data_and_model/mindtype-ai/Dockerfile` | Container image for the ML service |
| `data_and_model/mindtype-ai/requirements-ml.txt` | Minimal Python deps (no streamlit/plotly) |
