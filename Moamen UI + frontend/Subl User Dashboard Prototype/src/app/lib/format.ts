...
/**
 * Locale-aware formatting in a given timezone with custom Intl options. Use for
 * places that want a specific style (e.g. "Mon", "Jun 21", "3:04 PM") while
 * still honoring the user's timezone.
 */
export function formatInZone(
  ...
  if (!d) return "—";
  try {
    return new Intl.DateTimeFormat(locale, { timeZone: timezone || "UTC", ...options }).format(d);
  } catch {
