...
# 2. تدريب الموديل (اختياري قبل التشغيل)
python train_model.py

# 3. حفظ نموذج التجربة من notebook
...
├── 📄 train_model.py               ← درب الموديل وحفظ مخرجات الـ backend
├── 📄 save_model.py                ← درب الموديل التجريبي وحفظ artifact لمراقبة التوتر
│
├── 🔧 backend/
│   ├── __init__.py
...
│   └── stress_model.pkl            ← الموديل التجريبي لتحليل مستوى التوتر المتقدم
│
├── 📊 data/
...
القيم التالية مأخوذة من `model/metadata.json` بعد تشغيل `train_model.py`.
